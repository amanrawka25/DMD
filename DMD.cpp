        ...
        switch(fl)
		{
           ...
           case 11:i++;i++;
                  break;
							...
							if (charWide > 0) {
									strWidth += charWide ;
									//this->drawLine(bX + strWidth , bY, bX + strWidth , bY , GRAPHICS_INVERSE);
									//strWidth++;
							} else if (charWide < 0) {
									return;
							  }
...
int DMD::drawCharH(const int bX, const int bY, const unsigned char letter,const unsigned char letter1, const unsigned char letter2, const unsigned char letter3,  byte bGraphicsMode)
{
    if (bX > (DMD_PIXELS_ACROSS*DisplaysWide) || bY > (DMD_PIXELS_DOWN*DisplaysHigh) ) return -1;
    unsigned char c = letter;
